...
public class CiteTestSensor extends SensorPlugin {
	...
	public CiteTestSensor(SensorTaskService sensorTaskService, SensorConfiguration configuration)
	...
	private void scheduleAndAcceptTask(SubmitTaskingRequest submit, SensorTask submitTask)
			throws InvalidParameterValueException, SchedulerException {
		try {
	...
	/**
	 * inserito da Oscar Benedetti per avere il template
	 * 
	 * @return
	 */
	public DataRecordType getTemplate() {
		return getTaskingParametersAsDataRecord();
	}
	...
	private ReferenceType[] createReferences() {
		...
		/*
		 * We override the role here as for a DescribeResultAccess with procedure a service reference shall inform about
		 * the data service used in gerenal. There is no concrete data request to retrieve data as it would be when a
		 * DescribeResultAccess with a task id would be send. => see table 41 in SPS spec. 2.0
		 */
